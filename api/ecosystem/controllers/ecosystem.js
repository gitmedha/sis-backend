...
module.exports = {
     async createBulkEcosystem(ctx) {
        const { body } = ctx.request;
        
        try {
          const createdData = await strapi.services['ecosystem'].createMany(body);
          return createdData;
  ...
  try {
    ...
    if (searchField === "date_of_activity") {
      const records = await strapi.query('ecosystem').find({
...
async findDistinctField(ctx) {
  ...
  try {
    const values = await strapi.query('ecosystem').find({
      isactive: true,
      _limit: 1000000,
      _start: 0
    });
